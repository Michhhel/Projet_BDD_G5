...
class Inscription(db.Model):
    ...
    __table_args__ = (
        # Un étudiant = une seule inscription par année scolaire
...
# L'unicité (enseignant, matière, année) est gérée au niveau applicatif
# =============================================================================

class Enseignement(db.Model):
    __tablename__ = 'enseignement'
    __table_args__ = (
        # Unicité applicative renforcée par un index unique BDD
...
# Permet à un enseignement d'être dispensé dans plusieurs classes
# =============================================================================

class EnseignementClasse(db.Model):
    __tablename__ = 'enseignement_classe'
    __table_args__ = (
        # Un enseignement ne peut être rattaché deux fois à la même classe
...
# Unique par (enseignement_classe, libellé)
# =============================================================================
...
class TypeNote(db.Model):
    ...
    libelle         = db.Column(db.String(80), nullable=False)   # ex: "Devoir 1"
    ...
    notes_etudiants = db.relationship('NoteEtudiant', backref='type_note',
                                       lazy='dynamic')
    # Pas de cascade delete : les notes sont permanentes (règle métier)
...
class NoteEtudiant(db.Model):
    ...
    __table_args__ = (
        # Un étudiant ne peut avoir qu'une seule note par type d'évaluation
        UniqueConstraint('type_note_id', 'etudiant_id',
                         name='uq_note_etudiant_type_etu'),
        # Contrainte BDD : valeur entre 0 et 20
    ...
    # date_modification : mis à jour à chaque remplacement (UPDATE uniquement)
    ...
    def valeur_float(self):
        """Retourne la valeur en float pour les calculs statistiques."""
    ...
    def interdire_suppression(mapper, connection, target):
        """Signal SQLAlchemy déclenché avant tout DELETE sur cette table.
        Lève une exception pour bloquer toute suppression accidentelle."""
        raise RuntimeError(
...
# Enregistrement de l'event listener AVANT_DELETE pour NoteEtudiant
...
# Expiration configurable (défaut : 24h) – invalidé après utilisation
# =============================================================================
...
class CodeReinit(db.Model):
    ...
    def est_valide(self) -> bool:
        """Retourne True si le code n'a pas encore été utilisé et n'est pas expiré."""
        return (not self.utilise) and (datetime.utcnow() < self.date_expiration)

    @classmethod
    def creer(cls, etudiant_id: int, code: str) -> 'CodeReinit':
        """Crée un nouveau code de réinitialisation avec expiration automatique."""
        return cls(
    ...
    def invalider(self):
        """Marque le code comme utilisé (ne pas supprimer, garder la trace)."""
        self.utilise = True
...
# Accélère la recherche d'un étudiant par email (connexion)
Index('ix_etudiant_email', Etudiant.email)

# Accélère la recherche des inscriptions par année et classe
Index('ix_inscription_annee_classe', Inscription.annee_scolaire_id, Inscription.classe_id)

# Accélère la recherche des notes par type_note (stats)
Index('ix_note_type_note_id', NoteEtudiant.type_note_id)

# Accélère la recherche des codes non utilisés d'un étudiant
...
def init_db(app):
    ...
    À appeler dans create_app() : init_db(app)
    """
    db.init_app(app)
